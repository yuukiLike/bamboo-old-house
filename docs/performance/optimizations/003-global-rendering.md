...
- **保留原有完整呈现效果及一键恢复入口，禁止按性能读数自动降档。** 初版 `3ea7d29` 默认完整效果；2026-09-26 用户认可后明确授权先将性能版设为默认并推送，当前默认因此改为“稍柔和＋隔帧更新”。
...
2. 父版本的方向光阴影每次主场景渲染都刷新，即使暂停后风动时间、风强度、光源变换等输入都没有改变。仅在阴影输入完全一致时复用，可避免重复计算而不改变时序。
3. Three r185 点光源代码已跳过零贡献光源的阴影采样，但仍调用直接光 BRDF。只对内置标准/物理材质的零贡献点光源跳过该调用，保留非零贡献、其它灯光与原有材质补丁。
4. 减少渲染像素或隔帧更新动态方向光阴影有视觉代价，不能算作无损优化收益。初版默认关闭，经用户明确认可后改为默认开启，仍保留独立配置与完整效果。

## 验证计划

- 不使用 computer-use；通过源码、确定性测试、项目检查及构建验证。
- 验证阴影首次绘制、暂停/恢复、风雨渐变、时段渐变、视角切换、尺寸变化和配置切回；完整效果路径仍逐帧更新动态阴影。
...
- 完整效果的 DPR 上限仍为桌面 1.6、移动 1.25；方向光阴影尺寸仍为桌面 3072、移动 1536。模型、雨滴、湿润效果、叶片风动、灯光与接触阴影参数沿用父版本。
...
| 配置 | 当前默认（性能设置） | 完整效果选项 | 性能设置的可见代价 |
| --- | --- | --- | --- |
| 画面清晰度 | 稍柔和：在原 DPR 上乘 0.85 | 完整清晰 | 细竹叶及远处纹理略柔和；理论主画面像素数为原来的 72.25%，实际尺寸还会取整 |
| 日光/月光动态阴影 | 隔帧更新：稳定风动时每两帧刷新一次 | 每帧跟随 | 竹影最多落后一个渲染帧，大风或原帧率较低时可能不连贯；光源或风强度变化仍立即刷新 |

“恢复完整效果”同时切回完整清晰与每帧跟随，使用独立的 `FULL_RENDER_SETTINGS`，不会误回到性能默认值。设置在本次访问中跨视角/天气保持；刷新页面回到用户授权的性能设置。React 控件与场景初始化都读取同一个默认值，从首帧开始保持一致。分辨率改变时同步调整主画面、室内后处理和接触阴影目标，清除旧尺寸转场快照。文字控件的 CSS 尺寸不变。没有按帧时自动降档，也没有减少雨量的选项。
...
以下三份证据采集于实现提交 `3ea7d29`，分别为父版本、完整效果、稍柔和＋隔帧阴影，夹具哈希和六个资产哈希一致。它们保留采集时的源码哈希，不因后续默认选择改变而改写；当时的可选性能配置即当前产品默认配置。

- [父版本完整效果](./evidence/tidewater-03-before.json)
- [新版本完整效果](./evidence/tidewater-03-after-full.json)
- [新版本稍柔和＋隔帧阴影](./evidence/tidewater-03-after-combined.json)

| 每个稳定输入组合，30 次提交 | 父版本 | 新版本完整效果 | 新版本性能设置 |
| --- | ---: | ---: | ---: |
| 持续风动时方向光阴影刷新 | 30 | **30，保持原动态细节** | 15 |
| 暂停且输入不变时方向光阴影刷新 | 30 | **1** | 1 |

完整效果动态状态下，方向光阴影 draw/triangle 计数逐行与父版本相同；暂停状态下减少 29 次相同输入的重复提交。各视角均通过相同断言，视线以外的投影竹子仍保留。性能设置的动态阴影提交数和三角形提交数减半，不能解读为整体帧时减半。
...
| 示例视口，设备 DPR 2 | 完整效果绘制尺寸 | 性能默认绘制尺寸 |
| --- | --- | --- |
...
- `pnpm build` 通过；新旧版本均有原有大 chunk 提示。最新预览及固定父版本的 HTML、所引用 JS/CSS、建筑资源 HTTP 检查通过。初版 HTML 默认配置为 `full/full`；用户授权切换后重新检查为 `reduced/alternate`，保留“画面设置”入口。
- 默认选择变更后重新运行类型、lint、35 项测试、LOD 检查及生产构建；默认值断言更新为性能设置，独立验证“恢复完整效果”在桌面/移动 DPR 下仍精确返回原分辨率。渲染算法未变，不将此次默认切换报告为新的性能收益。
- **没有执行 computer-use、浏览器 GPU 性能采集、驱动着色器编译验证或像素对照。** 因此没有“8.3 Hz 已提升到多少”的结论，也没有将画质取舍算成无损优化收益。完整效果下持续动态场景的收益主要取决于点光源跳过分支在实际 GPU 上的效果，仍需同条件实看/采集。
...
输出使用独占创建，不覆盖已有证据；重测时换文件名。夹具默认仍固定为 `full` 以便做完整效果对照，`--profile combined` 对应当前产品默认值；`soft`、`alternate` 两个夹具档位可以单独检查清晰度或阴影选项。

本次本机对照入口：当前分支 `http://127.0.0.1:4175/?perf=1`；固定父提交 `b0cb10b` 为 `http://127.0.0.1:4178/?perf=1`。父版本工作树 `/tmp/bamboo-tidewater-stage2-b0cb10b` 与当前目录隔离。要做完整效果对照，先在新版本点击“恢复完整效果”，再保持同一设备/窗口/视角/时段/天气比较；当前性能默认与旧版有明确的画质取舍。同时运行多个 3D 标签页会争用 GPU，宜轮流查看。
