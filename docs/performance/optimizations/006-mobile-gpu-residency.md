...
> 2026-09-26 归档：动态播放时的 iPhone 持续掉帧仍未解决。文档先在 `3719ad4` 独立归档，随后按用户指令随本次源码提交交付所述实现，真机长期验收仍待继续。先读[调查检查点](../iphone-investigation-status.md)了解当前结论、代码状态与后续验证；本篇保留完整实验和各轮边界。加入静止按需绘制后的完整检查已增至 61 项测试；下面早期的 56 项为该轮验证记录。
