...
| 03 | `perf/tidewater-scene-optimizations-03-global-rendering` | 默认跳过零贡献点光源计算、复用输入未变的方向光阴影；全局提供用户主动选择的清晰度和阴影更新配置。真实模型的阴影提交夹具覆盖 2,720 个组合：默认动态阴影保持逐帧，静止输入 30 次重复绘制变为 1 次。可选隔帧模式为 15 次；这些是工作量计数，不是 FPS 提升。 | [第三阶段](./optimizations/003-global-rendering.md) |
...
- 记录当前清晰度和阴影配置。默认完整效果与用户主动选择的取舍分别比较，禁止将降档收益混入无损优化。
