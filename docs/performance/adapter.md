...
通用源码集中在 [`tools/scene-perf/`](../../tools/scene-perf/README.md)，可整体复制；浏览器核心与 React hook 的接入、依赖及拆卸步骤见该目录 README。通用采集器负责浏览器配置、导航、RAF、资源、User Timing、trace、原始数据和报告。**适配器声明项目何时就绪、如何读取诊断和状态；场景脚本声明点击哪些真实控件。** 换项目不需要修改通用 helper，也不需要安装监控 SDK。

本仓库的 `bamboo.cjs` 用于竹屋，`tools/scene-perf/examples/three-adapter.cjs` 提供另一项目的接入契约。示例尚未在第二个真实项目完成端到端验证；不能把它的存在当成移植已经验证。采集步骤和前后对比见[流水线说明](pipeline.md)。
...
复制 `tools/scene-perf/` 后可直接运行通用入口。内置 `load` 与 `cache` 都支持其他项目适配器；竹屋的 `journey`、`system`、`views` 包含项目控件，不能直接用于新项目：
...
这里的 `--instrumentation off` 只移除工具管理的 `perf=1` 参数，不禁止采集页面已有的原生 User Timing。需要业务阶段时可接入 `createPhaseRecorder`，使用 `--instrumentation on`，并通过可重复的 `--instrumentation-source FILE` 明确列出项目配置和真实 recorder 源文件；不能只对重导出的薄文件计算指纹。缺少来源依据时仍可采集，但不能确认可比收益。不要把新项目适配器冒充为 `id: 'bamboo'`。

通用 CLI 默认采用 `examples/browser-adapter.cjs`，无需向目标页面添加对象，适合先采导航、资源、缓存与 RAF。它仅等待 `document.readyState === 'complete'`，不代表 3D 就绪；要用这里的 Three.js 就绪与状态协议，必须显式选择 `three-adapter.cjs`。
...
适配器是本地 CommonJS 文件，通过 `--adapter FILE.cjs` 选择。runner 将路径转为绝对路径，再通过 `PERF_ADAPTER` 传给场景 helper；直接使用 helper 时，`PERF_ADAPTER` 必须为绝对路径。项目入口 `pnpm perf` 默认加载 `scripts/perf/adapters/bamboo.cjs`；独立通用入口默认加载工具内的 `examples/browser-adapter.cjs`。
...
| `nullableStateFields` | 可选，已声明状态字段的子集；仅这些字段的 `null` 是已知状态（例如设置面板关闭），`undefined` 仍缺失 |
| `frameCapacity` | 应用 `diagnostics.frames` 的实际最大容量；没有这份协议时为 `null` |
| `frameLimitation` | 写明应用帧起点、暖机省略、刷新频率或不可用等限制 |
| `phaseLimitation` / `measurePrefix` | 可选，说明业务阶段边界及原生 measure 名称前缀；报告不套用竹屋的解释 |
...
竹屋默认适配器保留原先的 `.scene-mount.ready`、淡入 opacity、`startupMs` 与“自由看看”控件条件，仍在静态降级时失败。必需状态为 `view/place/timeOfDay/soundEnabled/soundBusy/soundError/paused/panorama/settingsPanel/weather.preset`；`nullableStateFields: ['settingsPanel']` 声明 `settingsPanel: null` 表示面板关闭。可选状态为 `volume/weatherBusy/weatherError`。示例适配器要求 `scene/camera/animation/soundEnabled`，其中任何 `null` 都属于未知，不能确认可比。
...
const timings = createPhaseRecorder({
  namespace: 'project',
  enabled: () => new URLSearchParams(location.search).get('perf') === '1',
  readyPhase: 'startup.ready',
});

const finish = timings.beginPhase('model.load', { asset: 'house.glb' });
try {
...
不必为了拆分阶段重写加载器。只有原项目已经分别执行 fetch、缓冲拼接与 `parseAsync()` 时，才分别包住真实边界；阶段可能嵌套和并行，不能相加成总耗时。同步 render 的计时是 CPU 调用范围，音频恢复结束也不是扬声器首次发声。不要每帧写 User Timing；recorder 使用有界记录与明确截断计数。宿主卸载业务诊断时调用 `timings.dispose()`，只清理该实例的记录；React 面板卸载不会自动销毁这个独立 recorder。
...
首屏之外使用 `--flow custom --scenario /absolute/path/project-flow.cjs`。可以复制 `tools/scene-perf/cli/load-journey.cjs` 的结构，正确引用同目录 `scene-helpers.cjs`，替换真实控件、业务条件、别名和预期场景集合；复用 `createCollector(context, commands)`、`boundary()`、`state()`、`finish()`。使用 `try/finally` 调用 collector 的 `dispose()` 释放网络监听；先记录操作前边界，再执行真实操作和等待，不直接篡改页面状态来跳过用户路径。每次捕获须保留真实的首次或重复进入、声音状态和画质条件。
