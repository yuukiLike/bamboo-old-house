...
技术栈为 React、TypeScript、Three.js、Tailwind CSS、Vinext/Vite。当前场景版本为 `bamboo-2026-09-13-performance`，模型与场景布局沿用原项目提交 `1dfca58`。
...
本地生产构建已通过类型检查、lint、13项回归测试及浏览器检查，覆盖八个地点、六种观看方式和五时段 × 四天气组合，并检查390×844竖屏和667×375横屏。同机桌面采样中，原版首次切夜晚出现约 4.9 秒主线程长任务，优化后已检查的切换最长约 80 毫秒。该数字衡量切换阻塞，不是 FPS 或首次打开耗时，也不代表所有设备或线上部署状态。连续三轮切换后，画布、纹理、几何和着色器数量保持不变。
