...
const CORE_FLOWS = {
  load: { scenario: path.join(directory, 'load-journey.cjs') },
  cache: { scenario: path.join(directory, 'cache-journey.cjs') },
...
function help(defaults, flows) {
  ...
  --flow ${Object.keys(flows).join('|')} Default ${defaults.flow || 'load'}; cache repeats navigation
  --observe-ms N            Default 5000; integer 1000..30000 per view
  --instrumentation on|off   Default ${defaults.instrumentation || 'off'}; controls perf=1 business markers
  --instrumentation-source FILE  Repeat for all business instrumentation sources
                            Adds to entry-point defaults; content is fingerprinted
  --adapter FILE.cjs        Default ${defaults.adapter || 'examples/browser-adapter.cjs (document readiness only)'}
  --scenario FILE.cjs       Custom Browsertime script; implies --flow custom
                            unless --flow is explicitly provided; custom skips HAR
...
Document completion does not prove 3D readiness. Supply an application adapter
for scene readiness and renderer/state evidence. Missing conditions block comparison.
`;
}
...
function parse(argv, defaults, flows) {
  const options = {
    ...
    adapter: defaults.adapter || path.join(directory, '../examples/browser-adapter.cjs'),
    compare: null,
  ...
  const names = {
    ...
    '--instrumentation-source': 'instrumentationSources',
  };
  const seen = new Set();
  for (let i = 0; i < argv.length; i++) {
    ...
    if (seen.has(flag) && key !== 'instrumentationSources') throw new Error(`Repeated argument ${flag}`);
    seen.add(flag);
    const value = argv[++i];
    if (!value || value.startsWith('--')) throw new Error(`${flag} requires a value`);
    if (key === 'instrumentationSources') options.instrumentationSources.push(value);
    else options[key] = value;
  }
  if (seen.has('--scenario') && !seen.has('--flow')) options.flow = 'custom';
  for (const [key, choices] of Object.entries({
    profile: ['desktop', 'mobile'], mode: ['baseline', 'diagnostic'],
    flow: Object.keys(flows), instrumentation: ['on', 'off'],
  })) {
  ...
  const scenario = options.scenario || flows[options.flow].scenario;
  if (!scenario) throw new Error('The custom flow requires --scenario FILE.cjs');
  options.scenario = path.resolve(scenario);
  options.adapter = path.resolve(options.adapter);
  options.instrumentationSources = [...new Set(options.instrumentationSources.map(source => path.resolve(source)))].sort((a, b) => a.localeCompare(b));
  ...
  const bundled = Object.values(flows).some(flow => flow.scenario && options.scenario === path.resolve(flow.scenario));
  const selectedScenario = flows[options.flow].scenario;
  if (bundled && (!selectedScenario || options.scenario !== path.resolve(selectedScenario))) {
...
async function instrumentationFingerprint(options) {
  const sources = [];
  for (const source of options.instrumentationSources) {
    try {
      if (!(await stat(source)).isFile()) throw new Error('not a file');
      const content = await readFile(source);
      sources.push({ path: source, sha256: createHash('sha256').update(content).digest('hex'), bytes: content.length });
    } catch (error) { throw new Error(`Cannot read instrumentation source ${source}: ${error.message}`); }
  }
  // Paths describe provenance but must not change comparability when the toolkit
  // is copied to another checkout. Identical sources are hashed only once.
  const contents = [...new Set(sources.map(source => source.sha256))].sort((a, b) => a.localeCompare(b));
  return {
    instrumentationSources: sources,
    instrumentationSha256: options.instrumentation === 'on' && contents.length
      ? createHash('sha256').update(JSON.stringify(contents)).digest('hex') : null,
  };
}
...
export async function run(argv = process.argv.slice(2), defaults = {}) {
  const flows = { ...CORE_FLOWS, ...defaults.flows };
  if (argv.length === 1 && argv[0] === '--help') {
    console.log(help(defaults, flows));
    return;
  }
  const options = parse(argv, defaults, flows);
  const instrumentation = await instrumentationFingerprint(options);
  ...
  const adapterSourceSha256 = createHash('sha256').update(await readFile(loadedAdapter.path)).digest('hex');
  const adapter = { ...loadedAdapter.metadata, path: loadedAdapter.path,
    sourceSha256: adapterSourceSha256,
    // Include the functions actually injected into the browser, including when
    // the adapter file re-exports an example or builds its metadata elsewhere.
    sha256: createHash('sha256').update(JSON.stringify({ sourceSha256: adapterSourceSha256,
      browser: loadedAdapter.browser, metadata: loadedAdapter.metadata })).digest('hex') };
  const flowContract = Object.values(flows).find(flow => flow.scenario && options.scenario === path.resolve(flow.scenario));
  if (flowContract?.adapterId && adapter.id !== flowContract.adapterId) {
    throw new Error(`The bundled ${options.flow} flow requires adapter ${flowContract.adapterId}. Adapter ${adapter.id} requires --scenario with your project's UI actions; --flow load or cache can use any adapter.`);
  }
  const declaredScenes = require(options.scenario).expectedScenes;
  ...
  const manifest = {
    ...
    config: {
      ...options, ...instrumentation, adapter, viewport, requestedWindow: { width: 1440, height: 900 },
  ...
  try {
    ...
    manifest.config.networkCache.classifierSha256 = createHash('sha256').update(await readFile(path.join(directory, '../core/resource-cache.mjs'))).digest('hex');
    if (options.instrumentation === 'on' && !instrumentation.instrumentationSha256) {
      manifest.config.instrumentationNote = 'Business instrumentation source hashes are unavailable; reports remain available, but baseline comparison cannot verify instrumentation overhead. Provide --instrumentation-source for all instrumentation modules, or use --instrumentation off for a browser-probe baseline.';
