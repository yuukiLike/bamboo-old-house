/* Minimal load / lazy-interior journey. Interactions use real browser controls. */
// oxlint-disable-next-line typescript/no-require-imports -- Browsertime loads navigation scripts as CommonJS.
const { createCollector, TIMEOUT_MS, SETTLE_MS } = require('./scene-helpers.cjs');
const expectedScenes = ['initial-3d', 'free-outdoor', 'interior-first', 'outdoor-return', 'interior-repeat'];

module.exports = async function (context, commands) {
  const flow = process.env.PERF_FLOW || 'journey';
  if (!['load', 'journey'].includes(flow)) throw new Error('PERF_FLOW must be load or journey');
  const { boundary, finish, initial, observationMs } = await createCollector(context, commands);
  await initial();
  if (flow === 'load') return;
